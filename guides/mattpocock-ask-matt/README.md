# 在 CodeArts CLI 中使用 ask-matt

[English](README.en.md)

在工作阶段边界按有序决策树选择继续、清空、交接、子代理或压缩上下文。
...
同名时项目级优先。安装前检查 `ask-matt`；有冲突就停止，不能覆盖。
...
```text
请在当前项目安装并验证 Matt Pocock ask-matt，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path (Get-Location) ".codeartsdoer\skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("ask-matt")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\engineering\ask-matt") -Destination (Join-Path $target "ask-matt") -Recurse。源到目标为：skills/engineering/ask-matt -> .codeartsdoer/skills/ask-matt。
4. 在安装范围对应的隔离工作目录，逐行原样执行以下完整夹具；先创建夹具，再运行 CodeArts：
# 无需额外文件夹具；这是只读 smoke test。
...
6. 运行 codearts debug skill，确认 ask-matt 的 location 均在当前项目。
7. 原样运行：codearts run -m <外部模型ID> --format json "Explicitly use the ask-matt skill. We are at a phase boundary after implementation. The next phase is QA in this same directory; the implementation reasoning remains relevant, but the session is near its smart-zone limit. Apply the five-option ordered decision tree from PHASE-BOUNDARIES.md, name all five options, and recommend exactly one. Do not call another skill and do not modify files."
8. 只有 JSON 出现 completed Skill "ask-matt" 事件，且按顺序列出 Continue、/clear、/handoff、Subagent、/compact 并推荐 /compact才通过。
9. 报告准确文件和事件。卸载必须删除 .codeartsdoer/skills/ask-matt；若源码 checkout 是本次创建且未共享，也删除精确的 .tmp/mattpocock-skills-6654f6b。不得删除整个 skills 或 .tmp 父目录、其他 Skill、任何用户文件、codearts_cli.json、package.json 或凭据。
```

### 个人级提示词

```text
请为当前 Windows 用户安装并验证个人级 Matt Pocock ask-matt，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path $userRoot "skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("ask-matt")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\engineering\ask-matt") -Destination (Join-Path $target "ask-matt") -Recurse。源到目标为：skills/engineering/ask-matt -> $userRoot/skills/ask-matt。
...
7. 原样运行：codearts run -m <外部模型ID> --format json "Explicitly use the ask-matt skill. We are at a phase boundary after implementation. The next phase is QA in this same directory; the implementation reasoning remains relevant, but the session is near its smart-zone limit. Apply the five-option ordered decision tree from PHASE-BOUNDARIES.md, name all five options, and recommend exactly one. Do not call another skill and do not modify files."
8. 通过标准：completed Skill "ask-matt"，且按顺序列出 Continue、/clear、/handoff、Subagent、/compact 并推荐 /compact。
9. 卸载必须删除 $userRoot/skills/ask-matt；若源码 checkout 是本次创建且未共享，也删除隔离目录精确的 .tmp/mattpocock-skills-6654f6b。不得删除 $userRoot/skills 或 .tmp 父目录、package.json、codearts_cli.json、其他 Skill 或凭据。
```
...
必须同时看到目标 location、completed Skill "ask-matt"，以及按顺序列出 Continue、/clear、/handoff、Subagent、/compact 并推荐 /compact。没有 completed `skill` 事件时，即使文本看起来正确也不算通过。
...
只删除所选范围的 `ask-matt` 目录，保留父级 `skills`、其他 Skill、`package.json` 和 `codearts_cli.json`。删除前解析并核对精确绝对路径，删除后重跑 `codearts debug skill`。
...
只验证路由决策本身，没有验证它列出的每个下游 Skill。上游 `disable-model-invocation` 不作为 CodeArts 兼容断言；使用时要显式点名 `ask-matt`。
...
- [上游固定提交](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/engineering/ask-matt)
