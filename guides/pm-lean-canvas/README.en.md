# PM Lean Canvas on CodeArts CLI

[简体中文](README.md)

Install the verified `lean-canvas` skill from PM Skills to Create a nine-part Lean Canvas with key assumptions and minimum validation experiments.
...
```text
Install and verify the lean-canvas skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-lean-canvas and .codeartsdoer/skills/lean-canvas. Stop and report if either exists; never overwrite.
4. From the project root in PowerShell, run exactly:
   $source = Join-Path (Get-Location) ".codeartsdoer\vendor\pm-skills-lean-canvas"
   $target = Join-Path (Get-Location) ".codeartsdoer\skills\lean-canvas"
   ...
   git -C $source sparse-checkout set "pm-product-strategy/skills/lean-canvas"
   ...
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\lean-canvas") -Destination $target -Recurse
5. Run codearts debug skill and confirm lean-canvas resolves under this project's .codeartsdoer/skills/lean-canvas/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name lean-canvas and use no other tool. Do not access the network or write files. Create a concise Lean Canvas for an offline-first study planner for university students, with a freemium model and a paid 4 USD monthly sync tier. Use exact ASCII headings PROBLEM, SOLUTION, UVP, UNFAIR-ADVANTAGE, CUSTOMER-SEGMENTS, CHANNELS, REVENUE, COSTS, METRICS, ASSUMPTIONS, and EXPERIMENTS."
7. Passing criteria: There must be one completed `lean-canvas` skill event resolving to the selected native directory. The result must cover Problem, Solution, UVP, Unfair Advantage, Segments, Channels, Revenue, Costs, and Metrics, plus assumptions and experiments.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/lean-canvas and .codeartsdoer/vendor/pm-skills-lean-canvas. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped lean-canvas skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-lean-canvas and $userRoot/skills/lean-canvas. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   $source = Join-Path $userRoot "vendor\pm-skills-lean-canvas"
   $target = Join-Path $userRoot "skills\lean-canvas"
   if ((Test-Path -LiteralPath $source) -or (Test-Path -LiteralPath $target)) { throw "Source or target already exists; inspect it instead of overwriting." }
   New-Item -ItemType Directory -Path (Split-Path $source -Parent) -Force | Out-Null
   New-Item -ItemType Directory -Path (Split-Path $target -Parent) -Force | Out-Null
   git clone --filter=blob:none --no-checkout https://github.com/phuryn/pm-skills.git $source
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\lean-canvas") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/lean-canvas/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name lean-canvas and use no other tool. Do not access the network or write files. Create a concise Lean Canvas for an offline-first study planner for university students, with a freemium model and a paid 4 USD monthly sync tier. Use exact ASCII headings PROBLEM, SOLUTION, UVP, UNFAIR-ADVANTAGE, CUSTOMER-SEGMENTS, CHANNELS, REVENUE, COSTS, METRICS, ASSUMPTIONS, and EXPERIMENTS."
7. Passing criteria: There must be one completed `lean-canvas` skill event resolving to the selected native directory. The result must cover Problem, Solution, UVP, Unfair Advantage, Segments, Channels, Revenue, Costs, and Metrics, plus assumptions and experiments.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/lean-canvas and $userRoot/vendor/pm-skills-lean-canvas. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
$skills | Where-Object { $_.name -eq "lean-canvas" } | Select-Object name, location
```
...
codearts run --format json --model "mimo/mimo-v2.5" `
  "Call the skill tool exactly once with name lean-canvas and use no other tool. Do not access the network or write files. Create a concise Lean Canvas for an offline-first study planner for university students, with a freemium model and a paid 4 USD monthly sync tier. Use exact ASCII headings PROBLEM, SOLUTION, UVP, UNFAIR-ADVANTAGE, CUSTOMER-SEGMENTS, CHANNELS, REVENUE, COSTS, METRICS, ASSUMPTIONS, and EXPERIMENTS."
```

There must be one completed `lean-canvas` skill event resolving to the selected native directory. The result must cover Problem, Solution, UVP, Unfair Advantage, Segments, Channels, Revenue, Costs, and Metrics, plus assumptions and experiments. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name lean-canvas. Generate a Lean Canvas from these product and business constraints and identify the riskiest assumptions.
```
...
- `.codeartsdoer/skills/lean-canvas`
- `.codeartsdoer/vendor/pm-skills-lean-canvas`
...
Market size, cost, and revenue entries remain hypotheses and require validation with real data.
...
- Pin the commit and copy only `pm-product-strategy/skills/lean-canvas`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-product-strategy/skills/lean-canvas/SKILL.md)
