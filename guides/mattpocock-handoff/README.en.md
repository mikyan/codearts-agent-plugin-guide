# Use handoff with CodeArts CLI

[中文](README.md)

Compresses the current task into a redacted, actionable handoff document.
...
Project Skills override same-named user Skills. Stop on any collision in `handoff`.
...
```text
Install and verify project-scoped Matt Pocock handoff at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76.
...
3. Run exactly: $target = Join-Path (Get-Location) ".codeartsdoer\skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("handoff"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\productivity\handoff") -Destination (Join-Path $target "handoff") -Recurse. This copies skills/productivity/handoff -> .codeartsdoer/skills/handoff.
...
6. Run exactly: codearts run --auto -m <external-model-id> --format json "Use the handoff skill to create a handoff document for this task. Redact any secrets, save it to the temporary directory, and report the exact path."
7. Pass only with completed Skill "handoff" events and a handoff Markdown file in the temporary directory with sensitive values replaced by `<REDACTED>`.
8. Report exact files and events. Removal may delete only .codeartsdoer/skills/handoff; preserve the parent, other Skills, and all user files.
```

### User prompt

```text
Install and verify user-scoped Matt Pocock handoff at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76 for the current Windows user.
...
3. Run exactly: $target = Join-Path $userRoot "skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("handoff"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\productivity\handoff") -Destination (Join-Path $target "handoff") -Recurse. This copies skills/productivity/handoff -> $userRoot/skills/handoff.
...
6. Run exactly: codearts run --auto -m <external-model-id> --format json "Use the handoff skill to create a handoff document for this task. Redact any secrets, save it to the temporary directory, and report the exact path."
7. Pass only with completed Skill "handoff" and a handoff Markdown file in the temporary directory with sensitive values replaced by `<REDACTED>`.
8. Remove only $userRoot/skills/handoff; preserve the parent, package.json, codearts_cli.json, and unrelated Skills.
```
...
Copy-Item -LiteralPath (Join-Path $source "skills\productivity\handoff") -Destination (Join-Path $target "handoff") -Recurse
```
...
codearts run --auto -m "mimo/mimo-v2.5" --format json "Use the handoff skill to create a handoff document for this task. Redact any secrets, save it to the temporary directory, and report the exact path."
...
Require the expected locations, completed Skill "handoff", and a handoff Markdown file in the temporary directory with sensitive values replaced by `<REDACTED>`. A plausible answer without completed `skill` events is not a pass.

## Usage

```text
Use the handoff skill to create a handoff document for this task. Redact any secrets, save it to the temporary directory, and report the exact path.
```
...
Delete only the `handoff` directories in the chosen scope. Preserve the parent, unrelated Skills, `package.json`, and `codearts_cli.json`. Resolve exact absolute paths first, then rerun discovery.
...
- [Pinned upstream source](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/productivity/handoff)
