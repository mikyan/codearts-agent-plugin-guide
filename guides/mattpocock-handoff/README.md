# 在 CodeArts CLI 中使用 handoff

[English](README.en.md)

把当前任务压缩成脱敏、可继续执行的交接文档。
...
同名时项目级优先。安装前检查 `handoff`；有冲突就停止，不能覆盖。
...
```text
请在当前项目安装并验证 Matt Pocock handoff，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path (Get-Location) ".codeartsdoer\skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("handoff")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\productivity\handoff") -Destination (Join-Path $target "handoff") -Recurse。源到目标为：skills/productivity/handoff -> .codeartsdoer/skills/handoff。
...
5. 运行 codearts debug skill，确认 handoff 的 location 均在当前项目。
6. 原样运行：codearts run --auto -m <外部模型ID> --format json "Use the handoff skill to create a handoff document for this task. Redact any secrets, save it to the temporary directory, and report the exact path."
7. 只有 JSON 出现 completed Skill "handoff" 事件，且临时目录中生成交接 Markdown，敏感值被替换为 `<REDACTED>`才通过。
8. 报告准确文件和事件。卸载只能删除 .codeartsdoer/skills/handoff；不得删除整个 skills 目录、其他 Skill 或任何用户文件。
```

### 个人级提示词

```text
请为当前 Windows 用户安装并验证个人级 Matt Pocock handoff，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path $userRoot "skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("handoff")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\productivity\handoff") -Destination (Join-Path $target "handoff") -Recurse。源到目标为：skills/productivity/handoff -> $userRoot/skills/handoff。
...
6. 原样运行：codearts run --auto -m <外部模型ID> --format json "Use the handoff skill to create a handoff document for this task. Redact any secrets, save it to the temporary directory, and report the exact path."
7. 通过标准：completed Skill "handoff"，且临时目录中生成交接 Markdown，敏感值被替换为 `<REDACTED>`。
8. 卸载只能删除 $userRoot/skills/handoff；不得删除 $userRoot/skills、package.json、codearts_cli.json 或其他 Skill。
```
...
必须同时看到目标 location、completed Skill "handoff"，以及临时目录中生成交接 Markdown，敏感值被替换为 `<REDACTED>`。没有 completed `skill` 事件时，即使文本看起来正确也不算通过。
...
只删除所选范围的 `handoff` 目录，保留父级 `skills`、其他 Skill、`package.json` 和 `codearts_cli.json`。删除前解析并核对精确绝对路径，删除后重跑 `codearts debug skill`。
...
- [上游固定提交](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/productivity/handoff)
