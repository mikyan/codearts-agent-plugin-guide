# 在 CodeArts CLI 中使用 PM 定价策略

[English](README.en.md)

从 PM Skills 安装经过验证的 `pricing-strategy` Skill，依据价值、替代方案、成本与支付意愿设计定价模型、分层和验证实验。
...
```text
请在当前项目为 CodeArts CLI 安装并验证 phuryn/pm-skills 的 pricing-strategy Skill，固定到 Commit 18468a95b427e70e258b51389796367c6f684e7d。
...
3. 检查 .codeartsdoer/vendor/pm-skills-pricing-strategy 和 .codeartsdoer/skills/pricing-strategy。任一存在就停止并报告，不得覆盖。
...
5. 运行 codearts debug skill，确认 pricing-strategy 的 location 是当前项目 .codeartsdoer/skills/pricing-strategy/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步确认的模型，然后原样执行：
   codearts run --format json --model "<选择的模型>" "Call the skill tool exactly once with name pricing-strategy and use no other tool. Do not access the network or write files. Design pricing for an offline-first study planner. Facts: students save about 2 hours monthly, alternatives are free calendars and a 6 USD monthly planner, local core has near-zero marginal cost, encrypted cross-device sync costs 0.50 USD per active user monthly, and survey willingness to pay centers at 4 USD. Recommend model and value metric, define Free/Pro/University tiers, state annual discount, list 3 assumptions and 3 risks, and design one ethical pricing experiment with threshold. Use exact markers MODEL, VALUE-METRIC, FREE, PRO, UNIVERSITY, ANNUAL-DISCOUNT, ASSUMPTIONS, RISKS, and EXPERIMENT."
7. 通过标准：必须出现一次完成的 `pricing-strategy` Skill 事件并解析到所选原生目录；结果包含模型、价值计量单位、Free/Pro/University 三层、年度折扣、三个假设、三个风险，以及带阈值且不欺骗用户的定价实验。
8. 报告 Commit、源目录、目标目录、完成事件、来源路径和结果。卸载只能删除 .codeartsdoer/skills/pricing-strategy 与 .codeartsdoer/vendor/pm-skills-pricing-strategy；不得删除 .codeartsdoer 本身、package.json、codearts_cli.json 或其他 Skill。任一步失败必须停止，不得宣称成功。
```

### 个人级安装提示词

```text
请为当前 Windows 用户安装并验证个人级 phuryn/pm-skills pricing-strategy Skill，固定到 Commit 18468a95b427e70e258b51389796367c6f684e7d。

严格执行：
1. 在 PowerShell 设置 $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer"。只允许修改 $userRoot/vendor/pm-skills-pricing-strategy 和 $userRoot/skills/pricing-strategy；不得修改 $userRoot/package.json、codearts_cli.json、凭据、项目配置或全局软件。
2. 运行 codearts --version、git --version 和 codearts models；如果有多个模型，先让我选择 provider/model ID。
...
4. 在同一个 PowerShell 会话原样执行：
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\pricing-strategy") -Destination $target -Recurse
5. 切换到没有项目级同名 Skill 的干净目录，运行 codearts debug skill，确认 location 是 ~/.codeartsdoer/skills/pricing-strategy/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步模型，并在该干净目录原样执行：
   codearts run --format json --model "<选择的模型>" "Call the skill tool exactly once with name pricing-strategy and use no other tool. Do not access the network or write files. Design pricing for an offline-first study planner. Facts: students save about 2 hours monthly, alternatives are free calendars and a 6 USD monthly planner, local core has near-zero marginal cost, encrypted cross-device sync costs 0.50 USD per active user monthly, and survey willingness to pay centers at 4 USD. Recommend model and value metric, define Free/Pro/University tiers, state annual discount, list 3 assumptions and 3 risks, and design one ethical pricing experiment with threshold. Use exact markers MODEL, VALUE-METRIC, FREE, PRO, UNIVERSITY, ANNUAL-DISCOUNT, ASSUMPTIONS, RISKS, and EXPERIMENT."
7. 通过标准：必须出现一次完成的 `pricing-strategy` Skill 事件并解析到所选原生目录；结果包含模型、价值计量单位、Free/Pro/University 三层、年度折扣、三个假设、三个风险，以及带阈值且不欺骗用户的定价实验。
8. 报告 Commit、完成事件、来源路径和结果。卸载只能删除 $userRoot/skills/pricing-strategy 与 $userRoot/vendor/pm-skills-pricing-strategy；不得删除用户根 package.json、codearts_cli.json、凭据或其他 Skill。任一步失败必须停止，不得宣称成功。
```
...
必须出现一次完成的 `pricing-strategy` Skill 事件并解析到所选原生目录；结果包含模型、价值计量单位、Free/Pro/University 三层、年度折扣、三个假设、三个风险，以及带阈值且不欺骗用户的定价实验。 只有最终文本而没有完成的目标 `skill` 事件，不能视为通过。

## 使用

```text
Call the skill tool with name pricing-strategy. 根据这组价值、成本、竞品与支付意愿事实设计三层定价，并提出一个可判定实验。
```
...
没有实时竞品检索、真实 Van Westendorp 原始数据、税费或渠道抽成；输出是待验证方案，不是财务建议。

只验证 Windows 11、CodeArts CLI 26.8.1、`mimo/mimo-v2.5`、固定 Commit 和上述代表流程；未验证 Linux、macOS、其他模型、Claude `/commands` 或完整 PM 插件编排。

## 安全

- 固定 Commit，只复制 `pm-product-strategy/skills/pricing-strategy`；不运行上游 `validate_plugins.py`、测试脚本或其他插件内容。
...
- [固定版本 Skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-product-strategy/skills/pricing-strategy/SKILL.md)
