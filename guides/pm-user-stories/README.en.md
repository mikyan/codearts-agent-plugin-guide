# PM User Stories on CodeArts CLI

[简体中文](README.md)

Install the verified `user-stories` skill from PM Skills to Split a feature into user stories using 3C and INVEST, with testable acceptance criteria.
...
```text
Install and verify the user-stories skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-user-stories and .codeartsdoer/skills/user-stories. Stop and report if either exists; never overwrite.
4. From the project root in PowerShell, run exactly:
   $source = Join-Path (Get-Location) ".codeartsdoer\vendor\pm-skills-user-stories"
   $target = Join-Path (Get-Location) ".codeartsdoer\skills\user-stories"
   ...
   git -C $source sparse-checkout set "pm-execution/skills/user-stories"
   ...
   Copy-Item -LiteralPath (Join-Path $source "pm-execution\skills\user-stories") -Destination $target -Recurse
5. Run codearts debug skill and confirm user-stories resolves under this project's .codeartsdoer/skills/user-stories/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name user-stories and use no other tool. Do not access the network or write files. Create exactly 3 independent user stories for importing assignment deadlines into an offline-first study planner: student import, duplicate handling, and accessible error recovery. Each must include a title, an As a/I want/so that description, DESIGN=N/A, and 4 numbered acceptance criteria. Use exact markers STORY-1, STORY-2, STORY-3, and INVEST-CHECK."
7. Passing criteria: There must be one completed `user-stories` skill event resolving to the selected native directory. The result must contain exactly three independent stories, each with a title, As a/I want/so that description, Design N/A, and four numbered acceptance criteria, followed by an INVEST check. Punctuation or localization of `Design: N/A` does not affect the pass.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/user-stories and .codeartsdoer/vendor/pm-skills-user-stories. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped user-stories skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-user-stories and $userRoot/skills/user-stories. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   $source = Join-Path $userRoot "vendor\pm-skills-user-stories"
   $target = Join-Path $userRoot "skills\user-stories"
   if ((Test-Path -LiteralPath $source) -or (Test-Path -LiteralPath $target)) { throw "Source or target already exists; inspect it instead of overwriting." }
   New-Item -ItemType Directory -Path (Split-Path $source -Parent) -Force | Out-Null
   New-Item -ItemType Directory -Path (Split-Path $target -Parent) -Force | Out-Null
   git clone --filter=blob:none --no-checkout https://github.com/phuryn/pm-skills.git $source
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-execution\skills\user-stories") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/user-stories/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name user-stories and use no other tool. Do not access the network or write files. Create exactly 3 independent user stories for importing assignment deadlines into an offline-first study planner: student import, duplicate handling, and accessible error recovery. Each must include a title, an As a/I want/so that description, DESIGN=N/A, and 4 numbered acceptance criteria. Use exact markers STORY-1, STORY-2, STORY-3, and INVEST-CHECK."
7. Passing criteria: There must be one completed `user-stories` skill event resolving to the selected native directory. The result must contain exactly three independent stories, each with a title, As a/I want/so that description, Design N/A, and four numbered acceptance criteria, followed by an INVEST check. Punctuation or localization of `Design: N/A` does not affect the pass.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/user-stories and $userRoot/vendor/pm-skills-user-stories. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
$skills | Where-Object { $_.name -eq "user-stories" } | Select-Object name, location
```
...
codearts run --format json --model "mimo/mimo-v2.5" `
  "Call the skill tool exactly once with name user-stories and use no other tool. Do not access the network or write files. Create exactly 3 independent user stories for importing assignment deadlines into an offline-first study planner: student import, duplicate handling, and accessible error recovery. Each must include a title, an As a/I want/so that description, DESIGN=N/A, and 4 numbered acceptance criteria. Use exact markers STORY-1, STORY-2, STORY-3, and INVEST-CHECK."
```

There must be one completed `user-stories` skill event resolving to the selected native directory. The result must contain exactly three independent stories, each with a title, As a/I want/so that description, Design N/A, and four numbered acceptance criteria, followed by an INVEST check. Punctuation or localization of `Design: N/A` does not affect the pass. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name user-stories. Split this feature into three independent user stories that each fit within one sprint.
```
...
- `.codeartsdoer/skills/user-stories`
- `.codeartsdoer/vendor/pm-skills-user-stories`
...
All three results formatted `DESIGN=N/A` as a localized label, while the structure, story count, acceptance criteria, and INVEST check all passed.
...
- Pin the commit and copy only `pm-execution/skills/user-stories`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-execution/skills/user-stories/SKILL.md)
