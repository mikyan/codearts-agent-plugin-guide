# 在 CodeArts CLI 中使用 Humanizer

[English](README.en.md)

安装经过验证的 `humanizer` Skill，在保留事实与作者语气的同时去除常见 AI 写作痕迹。
...
```text
请在当前项目中为 CodeArts CLI 安装并验证 blader/humanizer 的 humanizer Skill，源码固定到 Commit ebf637bdae86b62a7b006c447bb98cfa0ccc979d。
...
3. 检查 .codeartsdoer/vendor/humanizer 和 .codeartsdoer/skills/humanizer。任一存在就停止并报告，不得覆盖。
...
5. 运行 codearts debug skill，确认 humanizer 的 location 位于当前项目 .codeartsdoer/skills/humanizer/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步确认的模型，然后原样执行：
   codearts run --format json --sandbox --model "<选择的模型>" "Call the skill tool exactly once with name humanizer. Do not use any other tool. Use embedded mode and output only the final rewrite of this sentence, preserving its launch year: Launched in 2024, this groundbreaking platform stands as a pivotal testament to innovation and promises a bright future."
7. 通过标准：JSON 中必须只有一次成功的 `humanizer` Skill 调用；最终结果保留 `2024`，并移除 `groundbreaking`、`pivotal`、`testament` 和 `bright future`。
...
```text
请为当前 Windows 用户安装并验证个人级 blader/humanizer humanizer Skill，源码固定到 Commit ebf637bdae86b62a7b006c447bb98cfa0ccc979d。

严格执行：
1. 在 PowerShell 执行 $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer"。只修改该目录下 vendor/humanizer 和 skills/humanizer；不要修改 $userRoot/package.json、codearts_cli.json、凭据、项目配置或全局软件。
2. 运行 codearts --version、git --version 和 codearts models；模型不明确时先询问我选择 provider/model ID。
3. 检查 $userRoot/vendor/humanizer 和 $userRoot/skills/humanizer。任一存在就停止并报告，不得覆盖。
4. 在同一个 PowerShell 会话原样执行：
   ...
   New-Item -ItemType Directory -Path $target -Force | Out-Null
   Copy-Item -LiteralPath (Join-Path $source "SKILL.md") -Destination (Join-Path $target "SKILL.md")
5. 在没有项目级同名 Skill 的目录运行 codearts debug skill，确认 humanizer 的 location 位于当前用户 .codeartsdoer/skills/humanizer/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步确认的模型，在同一目录原样执行：
   codearts run --format json --sandbox --model "<选择的模型>" "Call the skill tool exactly once with name humanizer. Do not use any other tool. Use embedded mode and output only the final rewrite of this sentence, preserving its launch year: Launched in 2024, this groundbreaking platform stands as a pivotal testament to innovation and promises a bright future."
7. 通过标准：JSON 中必须只有一次成功的 `humanizer` Skill 调用；最终结果保留 `2024`，并移除 `groundbreaking`、`pivotal`、`testament` 和 `bright future`。
8. 卸载只能移除 $userRoot/skills/humanizer 和 $userRoot/vendor/humanizer。报告工具事件与结果；任一步失败不得宣称成功。
```
...
JSON 中必须只有一次成功的 `humanizer` Skill 调用；最终结果保留 `2024`，并移除 `groundbreaking`、`pivotal`、`testament` 和 `bright future`。 只有最终文本但没有成功的 `skill` 工具事件，不能视为通过。
...
更新时先审查新 Commit，再移除所选范围内的旧 Skill 和专用 `vendor/humanizer`，按相同步骤重新安装并复测。不要复用浮动的 `main` 作为已验证版本。
...
| 固定源码 | [ebf637b](https://github.com/blader/humanizer/commit/ebf637bdae86b62a7b006c447bb98cfa0ccc979d) |
| 已验证 Skill | `humanizer` |
| 许可证 | MIT |
...
该安装在两个全新项目中复现，随后又从没有项目配置的目录验证个人级加载。真实 CodeArts 会话成功调用 `humanizer` 并完成上述代表任务；三个范围均完成回滚，回滚后 Skill 不再被发现。

Windows 默认 Git 检出会把上游 `skills/humanizer/SKILL.md` 符号链接变成只含 `../../SKILL.md` 的普通文本，因此本指南改为把仓库根 `SKILL.md` 复制到 CodeArts 原生目录，兼容状态为 `Adapter Required`。未验证整文件写回模式和仓库脚本。

## 安全说明

- 从固定 Commit 复制仓库根 `SKILL.md`；不要安装 Windows 下失效的符号链接，也不运行仓库中的其他代码。
...
- [固定版本 Skill](https://github.com/blader/humanizer/blob/ebf637bdae86b62a7b006c447bb98cfa0ccc979d/SKILL.md)
