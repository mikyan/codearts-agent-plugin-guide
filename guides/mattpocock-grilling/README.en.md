# Use grilling with CodeArts CLI

[中文](README.md)

Interviews you in numbered rounds with recommendations until a design decision tree is explicit.
...
Project Skills override same-named user Skills. Stop on any collision in `grilling`.
...
```text
Install and verify project-scoped Matt Pocock grilling at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76.
...
3. Run exactly: $target = Join-Path (Get-Location) ".codeartsdoer\skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("grilling"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\productivity\grilling") -Destination (Join-Path $target "grilling") -Recurse. This copies skills/productivity/grilling -> .codeartsdoer/skills/grilling.
...
6. Run exactly: codearts run -m <external-model-id> --format json "Use the grilling skill to interview me about designing a local-only CLI backup tool. Do not implement anything. Begin the first interview round now."
7. Pass only with completed Skill "grilling" events and numbered questions such as Q1, each with a recommended answer.
8. Report exact files and events. Removal may delete only .codeartsdoer/skills/grilling; preserve the parent, other Skills, and all user files.
```

### User prompt

```text
Install and verify user-scoped Matt Pocock grilling at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76 for the current Windows user.
...
3. Run exactly: $target = Join-Path $userRoot "skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("grilling"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\productivity\grilling") -Destination (Join-Path $target "grilling") -Recurse. This copies skills/productivity/grilling -> $userRoot/skills/grilling.
...
6. Run exactly: codearts run -m <external-model-id> --format json "Use the grilling skill to interview me about designing a local-only CLI backup tool. Do not implement anything. Begin the first interview round now."
7. Pass only with completed Skill "grilling" and numbered questions such as Q1, each with a recommended answer.
8. Remove only $userRoot/skills/grilling; preserve the parent, package.json, codearts_cli.json, and unrelated Skills.
```
...
codearts run -m "mimo/mimo-v2.5" --format json "Use the grilling skill to interview me about designing a local-only CLI backup tool. Do not implement anything. Begin the first interview round now."
...
Require the expected locations, completed Skill "grilling", and numbered questions such as Q1, each with a recommended answer. A plausible answer without completed `skill` events is not a pass.

## Usage

```text
Use the grilling skill to interview me about designing a local-only CLI backup tool. Do not implement anything. Begin the first interview round now.
```
...
Delete only the `grilling` directories in the chosen scope. Preserve the parent, unrelated Skills, `package.json`, and `codearts_cli.json`. Resolve exact absolute paths first, then rerun discovery.
...
- [Pinned upstream source](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/productivity/grilling)
