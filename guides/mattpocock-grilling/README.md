# 在 CodeArts CLI 中使用 grilling

[English](README.en.md)

通过分轮、编号并附推荐答案的追问，把设计决策树逐层问清。
...
同名时项目级优先。安装前检查 `grilling`；有冲突就停止，不能覆盖。
...
```text
请在当前项目安装并验证 Matt Pocock grilling，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path (Get-Location) ".codeartsdoer\skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("grilling")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\productivity\grilling") -Destination (Join-Path $target "grilling") -Recurse。源到目标为：skills/productivity/grilling -> .codeartsdoer/skills/grilling。
...
5. 运行 codearts debug skill，确认 grilling 的 location 均在当前项目。
6. 原样运行：codearts run -m <外部模型ID> --format json "Use the grilling skill to interview me about designing a local-only CLI backup tool. Do not implement anything. Begin the first interview round now."
7. 只有 JSON 出现 completed Skill "grilling" 事件，且出现编号问题（如 Q1）且每题带推荐答案才通过。
8. 报告准确文件和事件。卸载只能删除 .codeartsdoer/skills/grilling；不得删除整个 skills 目录、其他 Skill 或任何用户文件。
```

### 个人级提示词

```text
请为当前 Windows 用户安装并验证个人级 Matt Pocock grilling，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path $userRoot "skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("grilling")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\productivity\grilling") -Destination (Join-Path $target "grilling") -Recurse。源到目标为：skills/productivity/grilling -> $userRoot/skills/grilling。
...
6. 原样运行：codearts run -m <外部模型ID> --format json "Use the grilling skill to interview me about designing a local-only CLI backup tool. Do not implement anything. Begin the first interview round now."
7. 通过标准：completed Skill "grilling"，且出现编号问题（如 Q1）且每题带推荐答案。
8. 卸载只能删除 $userRoot/skills/grilling；不得删除 $userRoot/skills、package.json、codearts_cli.json 或其他 Skill。
```
...
必须同时看到目标 location、completed Skill "grilling"，以及出现编号问题（如 Q1）且每题带推荐答案。没有 completed `skill` 事件时，即使文本看起来正确也不算通过。
...
只删除所选范围的 `grilling` 目录，保留父级 `skills`、其他 Skill、`package.json` 和 `codearts_cli.json`。删除前解析并核对精确绝对路径，删除后重跑 `codearts debug skill`。
...
- [上游固定提交](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/productivity/grilling)
