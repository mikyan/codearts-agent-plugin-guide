# Use codebase-design with CodeArts CLI

[中文](README.md)

Reviews and improves code interfaces using deep-module and information-hiding principles.
...
Project Skills override same-named user Skills. Stop on any collision in `codebase-design`.
...
```text
Install and verify project-scoped Matt Pocock codebase-design at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76.
...
3. Run exactly: $target = Join-Path (Get-Location) ".codeartsdoer\skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("codebase-design"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\engineering\codebase-design") -Destination (Join-Path $target "codebase-design") -Recurse. This copies skills/engineering/codebase-design -> .codeartsdoer/skills/codebase-design.
...
6. Run exactly: codearts run -m <external-model-id> --format json "Use the codebase-design skill. Compare a shallow backup module exposing ten filesystem operations with a deep module exposing backup(source, destination) and restore(snapshot). Recommend one and explain why."
7. Pass only with completed Skill "codebase-design" events and an explicit shallow-versus-deep comparison and recommendation.
8. Report exact files and events. Removal may delete only .codeartsdoer/skills/codebase-design; preserve the parent, other Skills, and all user files.
```

### User prompt

```text
Install and verify user-scoped Matt Pocock codebase-design at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76 for the current Windows user.
...
3. Run exactly: $target = Join-Path $userRoot "skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("codebase-design"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\engineering\codebase-design") -Destination (Join-Path $target "codebase-design") -Recurse. This copies skills/engineering/codebase-design -> $userRoot/skills/codebase-design.
...
6. Run exactly: codearts run -m <external-model-id> --format json "Use the codebase-design skill. Compare a shallow backup module exposing ten filesystem operations with a deep module exposing backup(source, destination) and restore(snapshot). Recommend one and explain why."
7. Pass only with completed Skill "codebase-design" and an explicit shallow-versus-deep comparison and recommendation.
8. Remove only $userRoot/skills/codebase-design; preserve the parent, package.json, codearts_cli.json, and unrelated Skills.
```
...
Require the expected locations, completed Skill "codebase-design", and an explicit shallow-versus-deep comparison and recommendation. A plausible answer without completed `skill` events is not a pass.
...
Delete only the `codebase-design` directories in the chosen scope. Preserve the parent, unrelated Skills, `package.json`, and `codearts_cli.json`. Resolve exact absolute paths first, then rerun discovery.
...
- [Pinned upstream source](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/engineering/codebase-design)
