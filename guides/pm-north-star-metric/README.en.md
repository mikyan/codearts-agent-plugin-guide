# PM North Star Metric on CodeArts CLI

[简体中文](README.md)

Install the verified `north-star-metric` skill from PM Skills to classify the business game, select one customer-value metric, and build four actionable input metrics.
...
```text
Install and verify the north-star-metric skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-north-star-metric and .codeartsdoer/skills/north-star-metric. Stop and report if either exists; never overwrite.
4. From the project root in PowerShell, run exactly:
   $source = Join-Path (Get-Location) ".codeartsdoer\vendor\pm-skills-north-star-metric"
   ...
   git -C $source sparse-checkout set "pm-marketing-growth/skills/north-star-metric"
   ...
   Copy-Item -LiteralPath (Join-Path $source "pm-marketing-growth\skills\north-star-metric") -Destination $target -Recurse
5. Run codearts debug skill and confirm north-star-metric resolves under this project's .codeartsdoer/skills/north-star-metric/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name north-star-metric and use no other tool. Do not access the network or write files. For an offline-first study planner whose value is helping university students complete planned study work on time, classify the business game, choose exactly one customer-centric North Star Metric, assess it against all 7 stated criteria, and define exactly 4 measurable input metrics with causal explanations. Revenue and app opens are not acceptable as the North Star. Use exact markers BUSINESS-GAME, NORTH-STAR, CRITERIA-1-7, INPUT-1, INPUT-2, INPUT-3, and INPUT-4."
7. Passing criteria: There must be one completed `north-star-metric` skill event resolving to the selected native directory. The result must classify the business as a Productivity Game, choose exactly one customer-value North Star Metric, assess all seven criteria, and provide four measurable input metrics with causal explanations.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/north-star-metric and .codeartsdoer/vendor/pm-skills-north-star-metric. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped north-star-metric skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-north-star-metric and $userRoot/skills/north-star-metric. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   $source = Join-Path $userRoot "vendor\pm-skills-north-star-metric"
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-marketing-growth\skills\north-star-metric") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/north-star-metric/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name north-star-metric and use no other tool. Do not access the network or write files. For an offline-first study planner whose value is helping university students complete planned study work on time, classify the business game, choose exactly one customer-centric North Star Metric, assess it against all 7 stated criteria, and define exactly 4 measurable input metrics with causal explanations. Revenue and app opens are not acceptable as the North Star. Use exact markers BUSINESS-GAME, NORTH-STAR, CRITERIA-1-7, INPUT-1, INPUT-2, INPUT-3, and INPUT-4."
7. Passing criteria: There must be one completed `north-star-metric` skill event resolving to the selected native directory. The result must classify the business as a Productivity Game, choose exactly one customer-value North Star Metric, assess all seven criteria, and provide four measurable input metrics with causal explanations.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/north-star-metric and $userRoot/vendor/pm-skills-north-star-metric. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
$skills | Where-Object { $_.name -eq "north-star-metric" } | Select-Object name, location
```
...
codearts run --format json --model "mimo/mimo-v2.5" `
  "Call the skill tool exactly once with name north-star-metric and use no other tool. Do not access the network or write files. For an offline-first study planner whose value is helping university students complete planned study work on time, classify the business game, choose exactly one customer-centric North Star Metric, assess it against all 7 stated criteria, and define exactly 4 measurable input metrics with causal explanations. Revenue and app opens are not acceptable as the North Star. Use exact markers BUSINESS-GAME, NORTH-STAR, CRITERIA-1-7, INPUT-1, INPUT-2, INPUT-3, and INPUT-4."
```

There must be one completed `north-star-metric` skill event resolving to the selected native directory. The result must classify the business as a Productivity Game, choose exactly one customer-value North Star Metric, assess all seven criteria, and provide four measurable input metrics with causal explanations. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name north-star-metric. Choose one customer-value North Star Metric for this product and the four input metrics most likely to move it.
```
...
- `.codeartsdoer/skills/north-star-metric`
- `.codeartsdoer/vendor/pm-skills-north-star-metric`
...
| Verified skill | `north-star-metric` |
| License | MIT |
...
Only framework generation was verified; historical data was not used to test leading power, manipulability, or correlation with long-term revenue.
...
- Pin the commit and copy only `pm-marketing-growth/skills/north-star-metric`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-marketing-growth/skills/north-star-metric/SKILL.md)
