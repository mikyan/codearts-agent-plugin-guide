# PM Value Proposition on CodeArts CLI

[简体中文](README.md)

Install the verified `value-proposition` skill from PM Skills to Define customer, problem, current state, solution, future state, and alternatives with a six-part JTBD template.
...
```text
Install and verify the value-proposition skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-value-proposition and .codeartsdoer/skills/value-proposition. Stop and report if either exists; never overwrite.
4. From the project root in PowerShell, run exactly:
   $source = Join-Path (Get-Location) ".codeartsdoer\vendor\pm-skills-value-proposition"
   $target = Join-Path (Get-Location) ".codeartsdoer\skills\value-proposition"
   ...
   git -C $source sparse-checkout set "pm-product-strategy/skills/value-proposition"
   ...
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\value-proposition") -Destination $target -Recurse
5. Run codearts debug skill and confirm value-proposition resolves under this project's .codeartsdoer/skills/value-proposition/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name value-proposition and use no other tool. Do not access the network or write files. Design a value proposition for privacy-conscious university students using an offline-first study planner. Current alternatives are paper planners, calendar apps, and the university LMS; the product imports deadlines locally and generates a daily next-action list. Use the exact ASCII section markers WHO, WHY, WHAT-BEFORE, HOW, WHAT-AFTER, ALTERNATIVES, VALUE-PROP, and POSITIONING."
7. Passing criteria: There must be one completed `value-proposition` skill event resolving to the selected native directory. The result must cover Who, Why, What Before, How, What After, and Alternatives, plus concise value-proposition and positioning statements.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/value-proposition and .codeartsdoer/vendor/pm-skills-value-proposition. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped value-proposition skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-value-proposition and $userRoot/skills/value-proposition. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   $source = Join-Path $userRoot "vendor\pm-skills-value-proposition"
   $target = Join-Path $userRoot "skills\value-proposition"
   if ((Test-Path -LiteralPath $source) -or (Test-Path -LiteralPath $target)) { throw "Source or target already exists; inspect it instead of overwriting." }
   New-Item -ItemType Directory -Path (Split-Path $source -Parent) -Force | Out-Null
   New-Item -ItemType Directory -Path (Split-Path $target -Parent) -Force | Out-Null
   git clone --filter=blob:none --no-checkout https://github.com/phuryn/pm-skills.git $source
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\value-proposition") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/value-proposition/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name value-proposition and use no other tool. Do not access the network or write files. Design a value proposition for privacy-conscious university students using an offline-first study planner. Current alternatives are paper planners, calendar apps, and the university LMS; the product imports deadlines locally and generates a daily next-action list. Use the exact ASCII section markers WHO, WHY, WHAT-BEFORE, HOW, WHAT-AFTER, ALTERNATIVES, VALUE-PROP, and POSITIONING."
7. Passing criteria: There must be one completed `value-proposition` skill event resolving to the selected native directory. The result must cover Who, Why, What Before, How, What After, and Alternatives, plus concise value-proposition and positioning statements.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/value-proposition and $userRoot/vendor/pm-skills-value-proposition. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
$skills | Where-Object { $_.name -eq "value-proposition" } | Select-Object name, location
```
...
codearts run --format json --model "mimo/mimo-v2.5" `
  "Call the skill tool exactly once with name value-proposition and use no other tool. Do not access the network or write files. Design a value proposition for privacy-conscious university students using an offline-first study planner. Current alternatives are paper planners, calendar apps, and the university LMS; the product imports deadlines locally and generates a daily next-action list. Use the exact ASCII section markers WHO, WHY, WHAT-BEFORE, HOW, WHAT-AFTER, ALTERNATIVES, VALUE-PROP, and POSITIONING."
```

There must be one completed `value-proposition` skill event resolving to the selected native directory. The result must cover Who, Why, What Before, How, What After, and Alternatives, plus concise value-proposition and positioning statements. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name value-proposition. Design a six-part JTBD value proposition and positioning statement for this segment.
```
...
- `.codeartsdoer/skills/value-proposition`
- `.codeartsdoer/vendor/pm-skills-value-proposition`
...
| Verified skill | `value-proposition` |
| License | MIT |
...
Only text generation from complete local context was verified; external templates, market research, and customer validation were not tested.
...
- Pin the commit and copy only `pm-product-strategy/skills/value-proposition`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-product-strategy/skills/value-proposition/SKILL.md)
