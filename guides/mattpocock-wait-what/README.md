# 在 CodeArts CLI 中使用 wait-what

[English](README.en.md)

根据当前上下文把难懂术语重新讲清，并用具体例子检查理解。
...
同名时项目级优先。安装前检查 `wait-what`；有冲突就停止，不能覆盖。
...
```text
请在当前项目安装并验证 Matt Pocock wait-what，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path (Get-Location) ".codeartsdoer\skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("wait-what")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\productivity\wait-what") -Destination (Join-Path $target "wait-what") -Recurse。源到目标为：skills/productivity/wait-what -> .codeartsdoer/skills/wait-what。
...
5. 运行 codearts debug skill，确认 wait-what 的 location 均在当前项目。
6. 原样运行：codearts run -m <外部模型ID> --format json "Use the wait-what skill. Read CONTEXT.md and explain DeliveryAttempt to a newcomer using a concrete example, then check understanding."
7. 只有 JSON 出现 completed Skill "wait-what" 事件，且解释 DeliveryAttempt 并给出具体例子才通过。验证前在隔离目录准备 CONTEXT.md；不得使用真实凭据或生产数据。
8. 报告准确文件和事件。卸载只能删除 .codeartsdoer/skills/wait-what；不得删除整个 skills 目录、其他 Skill 或任何用户文件。
```

### 个人级提示词

```text
请为当前 Windows 用户安装并验证个人级 Matt Pocock wait-what，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path $userRoot "skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("wait-what")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\productivity\wait-what") -Destination (Join-Path $target "wait-what") -Recurse。源到目标为：skills/productivity/wait-what -> $userRoot/skills/wait-what。
...
6. 原样运行：codearts run -m <外部模型ID> --format json "Use the wait-what skill. Read CONTEXT.md and explain DeliveryAttempt to a newcomer using a concrete example, then check understanding."
7. 通过标准：completed Skill "wait-what"，且解释 DeliveryAttempt 并给出具体例子。验证前在隔离目录准备 CONTEXT.md；不得使用真实凭据或生产数据。
8. 卸载只能删除 $userRoot/skills/wait-what；不得删除 $userRoot/skills、package.json、codearts_cli.json 或其他 Skill。
```
...
上游是纯内容包；固定版本没有依赖安装或生命周期脚本。验证前在隔离目录准备 CONTEXT.md；不得使用真实凭据或生产数据。
...
必须同时看到目标 location、completed Skill "wait-what"，以及解释 DeliveryAttempt 并给出具体例子。没有 completed `skill` 事件时，即使文本看起来正确也不算通过。
...
只删除所选范围的 `wait-what` 目录，保留父级 `skills`、其他 Skill、`package.json` 和 `codearts_cli.json`。删除前解析并核对精确绝对路径，删除后重跑 `codearts debug skill`。
...
- [上游固定提交](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/productivity/wait-what)
