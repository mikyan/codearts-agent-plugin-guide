# PM SWOT Analysis on CodeArts CLI

[简体中文](README.md)

Install the verified `swot-analysis` skill from PM Skills to Turn internal strengths and weaknesses plus external opportunities and threats into prioritized actions.
...
```text
Install and verify the swot-analysis skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-swot-analysis and .codeartsdoer/skills/swot-analysis. Stop and report if either exists; never overwrite.
4. From the project root in PowerShell, run exactly:
   $source = Join-Path (Get-Location) ".codeartsdoer\vendor\pm-skills-swot-analysis"
   $target = Join-Path (Get-Location) ".codeartsdoer\skills\swot-analysis"
   ...
   git -C $source sparse-checkout set "pm-product-strategy/skills/swot-analysis"
   ...
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\swot-analysis") -Destination $target -Recurse
5. Run codearts debug skill and confirm swot-analysis resolves under this project's .codeartsdoer/skills/swot-analysis/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name swot-analysis and use no other tool. Do not access the network or write files. Produce a concise SWOT for an offline-first study planner. Facts: two-person team, privacy-first local storage, no brand awareness, university LMS incumbents, and increasing student concern about cloud privacy. Include at least 3 items per quadrant and 3 prioritized actions. Use exact markers STRENGTHS, WEAKNESSES, OPPORTUNITIES, THREATS, and ACTIONS."
7. Passing criteria: There must be one completed `swot-analysis` skill event resolving to the selected native directory. Each quadrant must contain at least three items, followed by at least three prioritized actions.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/swot-analysis and .codeartsdoer/vendor/pm-skills-swot-analysis. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped swot-analysis skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-swot-analysis and $userRoot/skills/swot-analysis. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   $source = Join-Path $userRoot "vendor\pm-skills-swot-analysis"
   $target = Join-Path $userRoot "skills\swot-analysis"
   if ((Test-Path -LiteralPath $source) -or (Test-Path -LiteralPath $target)) { throw "Source or target already exists; inspect it instead of overwriting." }
   New-Item -ItemType Directory -Path (Split-Path $source -Parent) -Force | Out-Null
   New-Item -ItemType Directory -Path (Split-Path $target -Parent) -Force | Out-Null
   git clone --filter=blob:none --no-checkout https://github.com/phuryn/pm-skills.git $source
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\swot-analysis") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/swot-analysis/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name swot-analysis and use no other tool. Do not access the network or write files. Produce a concise SWOT for an offline-first study planner. Facts: two-person team, privacy-first local storage, no brand awareness, university LMS incumbents, and increasing student concern about cloud privacy. Include at least 3 items per quadrant and 3 prioritized actions. Use exact markers STRENGTHS, WEAKNESSES, OPPORTUNITIES, THREATS, and ACTIONS."
7. Passing criteria: There must be one completed `swot-analysis` skill event resolving to the selected native directory. Each quadrant must contain at least three items, followed by at least three prioritized actions.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/swot-analysis and $userRoot/vendor/pm-skills-swot-analysis. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
$skills | Where-Object { $_.name -eq "swot-analysis" } | Select-Object name, location
```
...
codearts run --format json --model "mimo/mimo-v2.5" `
  "Call the skill tool exactly once with name swot-analysis and use no other tool. Do not access the network or write files. Produce a concise SWOT for an offline-first study planner. Facts: two-person team, privacy-first local storage, no brand awareness, university LMS incumbents, and increasing student concern about cloud privacy. Include at least 3 items per quadrant and 3 prioritized actions. Use exact markers STRENGTHS, WEAKNESSES, OPPORTUNITIES, THREATS, and ACTIONS."
```

There must be one completed `swot-analysis` skill event resolving to the selected native directory. Each quadrant must contain at least three items, followed by at least three prioritized actions. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name swot-analysis. Use only these known facts for a SWOT, then turn cross-quadrant insights into the top three actions.
```
...
- `.codeartsdoer/skills/swot-analysis`
- `.codeartsdoer/vendor/pm-skills-swot-analysis`
...
No external market research was performed; with incomplete input facts, the output is only a working hypothesis.
...
- Pin the commit and copy only `pm-product-strategy/skills/swot-analysis`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-product-strategy/skills/swot-analysis/SKILL.md)
