# 在 CodeArts CLI 中使用 PM 产品策略画布

[English](README.en.md)

从 PM Skills 安装经过验证的 `product-strategy` Skill，用九区画布把愿景、细分、价值、取舍、指标、增长、能力和防御性连成一致策略。
...
```text
请在当前项目为 CodeArts CLI 安装并验证 phuryn/pm-skills 的 product-strategy Skill，固定到 Commit 18468a95b427e70e258b51389796367c6f684e7d。
...
3. 检查 .codeartsdoer/vendor/pm-skills-product-strategy 和 .codeartsdoer/skills/product-strategy。任一存在就停止并报告，不得覆盖。
...
5. 运行 codearts debug skill，确认 product-strategy 的 location 是当前项目 .codeartsdoer/skills/product-strategy/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步确认的模型，然后原样执行：
   codearts run --format json --model "<选择的模型>" "Call the skill tool exactly once with name product-strategy and use no other tool. Do not access the network or write files. Create a coherent Product Strategy Canvas for a two-person team building an offline-first study planner for privacy-conscious university students. Facts: local deadline import, daily next action, freemium plus 4 USD sync, no brand, 12-month runway, and calendar/LMS alternatives. Cover all 9 canvas sections, name 3 explicit trade-offs, choose one North Star and one quarterly OMTM, identify build-versus-partner capabilities, state defensibility, surface 3 critical hypotheses, and give one low-effort experiment per hypothesis. Use exact markers VISION, SEGMENTS, RELATIVE-COSTS, VALUE-PROPOSITION, TRADE-OFFS, METRICS, GROWTH, CAPABILITIES, CANT-WONT, HYPOTHESES, and EXPERIMENTS."
7. 通过标准：必须出现一次完成的 `product-strategy` Skill 事件并解析到所选原生目录；结果覆盖九个画布区、三个明确取舍、单一北极星与季度 OMTM、构建/合作能力、防御性、三个关键假设及各自的低成本实验。
8. 报告 Commit、源目录、目标目录、完成事件、来源路径和结果。卸载只能删除 .codeartsdoer/skills/product-strategy 与 .codeartsdoer/vendor/pm-skills-product-strategy；不得删除 .codeartsdoer 本身、package.json、codearts_cli.json 或其他 Skill。任一步失败必须停止，不得宣称成功。
```

### 个人级安装提示词

```text
请为当前 Windows 用户安装并验证个人级 phuryn/pm-skills product-strategy Skill，固定到 Commit 18468a95b427e70e258b51389796367c6f684e7d。

严格执行：
1. 在 PowerShell 设置 $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer"。只允许修改 $userRoot/vendor/pm-skills-product-strategy 和 $userRoot/skills/product-strategy；不得修改 $userRoot/package.json、codearts_cli.json、凭据、项目配置或全局软件。
2. 运行 codearts --version、git --version 和 codearts models；如果有多个模型，先让我选择 provider/model ID。
...
4. 在同一个 PowerShell 会话原样执行：
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-product-strategy\skills\product-strategy") -Destination $target -Recurse
5. 切换到没有项目级同名 Skill 的干净目录，运行 codearts debug skill，确认 location 是 ~/.codeartsdoer/skills/product-strategy/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步模型，并在该干净目录原样执行：
   codearts run --format json --model "<选择的模型>" "Call the skill tool exactly once with name product-strategy and use no other tool. Do not access the network or write files. Create a coherent Product Strategy Canvas for a two-person team building an offline-first study planner for privacy-conscious university students. Facts: local deadline import, daily next action, freemium plus 4 USD sync, no brand, 12-month runway, and calendar/LMS alternatives. Cover all 9 canvas sections, name 3 explicit trade-offs, choose one North Star and one quarterly OMTM, identify build-versus-partner capabilities, state defensibility, surface 3 critical hypotheses, and give one low-effort experiment per hypothesis. Use exact markers VISION, SEGMENTS, RELATIVE-COSTS, VALUE-PROPOSITION, TRADE-OFFS, METRICS, GROWTH, CAPABILITIES, CANT-WONT, HYPOTHESES, and EXPERIMENTS."
7. 通过标准：必须出现一次完成的 `product-strategy` Skill 事件并解析到所选原生目录；结果覆盖九个画布区、三个明确取舍、单一北极星与季度 OMTM、构建/合作能力、防御性、三个关键假设及各自的低成本实验。
8. 报告 Commit、完成事件、来源路径和结果。卸载只能删除 $userRoot/skills/product-strategy 与 $userRoot/vendor/pm-skills-product-strategy；不得删除用户根 package.json、codearts_cli.json、凭据或其他 Skill。任一步失败必须停止，不得宣称成功。
```
...
必须出现一次完成的 `product-strategy` Skill 事件并解析到所选原生目录；结果覆盖九个画布区、三个明确取舍、单一北极星与季度 OMTM、构建/合作能力、防御性、三个关键假设及各自的低成本实验。 只有最终文本而没有完成的目标 `skill` 事件，不能视为通过。

## 使用

```text
Call the skill tool with name product-strategy. 根据这些约束生成九区产品策略画布，检查各区是否互相强化，并优先验证三个关键假设。
```
...
没有外部市场、财务或客户证据；画布中的定位、防御性和增长假设必须由真实研究与实验验证。

只验证 Windows 11、CodeArts CLI 26.8.1、`mimo/mimo-v2.5`、固定 Commit 和上述代表流程；未验证 Linux、macOS、其他模型、Claude `/commands` 或完整 PM 插件编排。

## 安全

- 固定 Commit，只复制 `pm-product-strategy/skills/product-strategy`；不运行上游 `validate_plugins.py`、测试脚本或其他插件内容。
...
- [固定版本 Skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-product-strategy/skills/product-strategy/SKILL.md)
