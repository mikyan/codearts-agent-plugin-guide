# PM Market Sizing on CodeArts CLI

[简体中文](README.md)

Install the verified `market-sizing` skill from PM Skills to estimate TAM, SAM, and SOM through top-down and bottom-up paths while making assumptions explicit.
...
```text
Install and verify the market-sizing skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-market-sizing and .codeartsdoer/skills/market-sizing. Stop and report if either exists; never overwrite.
...
5. Run codearts debug skill and confirm market-sizing resolves under this project's .codeartsdoer/skills/market-sizing/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name market-sizing and use no other tool. Do not access the network or write files. Use only these supplied facts for an offline-first student planner: 1,000,000 target students, annual willingness to pay 48 USD, 30% reachable English-speaking segment, and a realistic 5% three-year share of SAM. The independent top-down source estimate is 50 million USD annual category spend. Calculate TAM, SAM, and SOM, reconcile top-down and bottom-up TAM, list assumptions with confidence, and give one validation test. Use exact markers MARKET-DEFINITION, TOP-DOWN, BOTTOM-UP, TAM, SAM, SOM, RECONCILIATION, ASSUMPTIONS, and VALIDATION."
7. Passing criteria: There must be one completed `market-sizing` skill event resolving to the selected native directory. The result must include market boundaries, top-down and bottom-up estimates, TAM/SAM/SOM, reconciliation of the two paths, confidence-labeled assumptions, and one validation test.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/market-sizing and .codeartsdoer/vendor/pm-skills-market-sizing. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped market-sizing skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-market-sizing and $userRoot/skills/market-sizing. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-market-research\skills\market-sizing") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/market-sizing/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name market-sizing and use no other tool. Do not access the network or write files. Use only these supplied facts for an offline-first student planner: 1,000,000 target students, annual willingness to pay 48 USD, 30% reachable English-speaking segment, and a realistic 5% three-year share of SAM. The independent top-down source estimate is 50 million USD annual category spend. Calculate TAM, SAM, and SOM, reconcile top-down and bottom-up TAM, list assumptions with confidence, and give one validation test. Use exact markers MARKET-DEFINITION, TOP-DOWN, BOTTOM-UP, TAM, SAM, SOM, RECONCILIATION, ASSUMPTIONS, and VALIDATION."
7. Passing criteria: There must be one completed `market-sizing` skill event resolving to the selected native directory. The result must include market boundaries, top-down and bottom-up estimates, TAM/SAM/SOM, reconciliation of the two paths, confidence-labeled assumptions, and one validation test.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/market-sizing and $userRoot/vendor/pm-skills-market-sizing. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
There must be one completed `market-sizing` skill event resolving to the selected native directory. The result must include market boundaries, top-down and bottom-up estimates, TAM/SAM/SOM, reconciliation of the two paths, confidence-labeled assumptions, and one validation test. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name market-sizing. Use only these known customer, price, and reachability facts to estimate TAM, SAM, and SOM and identify the weakest assumptions.
```
...
The test forbade network access and verified only estimates from user-supplied data; real decisions still require source, currency, willingness-to-pay, and acquisition validation.
...
- Pin the commit and copy only `pm-market-research/skills/market-sizing`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-market-research/skills/market-sizing/SKILL.md)
