# Use diagnosing-bugs with CodeArts CLI

[中文](README.md)

Builds a repeatable failing loop and root-cause evidence before applying the smallest fix.
...
Project Skills override same-named user Skills. Stop on any collision in `diagnosing-bugs`.
...
```text
Install and verify project-scoped Matt Pocock diagnosing-bugs at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76.
...
3. Run exactly: $target = Join-Path (Get-Location) ".codeartsdoer\skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("diagnosing-bugs"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\engineering\diagnosing-bugs") -Destination (Join-Path $target "diagnosing-bugs") -Recurse. This copies skills/engineering/diagnosing-bugs -> .codeartsdoer/skills/diagnosing-bugs.
...
6. Run exactly: codearts run --auto -m <external-model-id> --format json "Use the diagnosing-bugs skill. Run node --test to reproduce the bug, diagnose the root cause, apply the smallest fix, and rerun the same command. Do not add dependencies."
7. Pass only with completed Skill "diagnosing-bugs" events and the same command failing then passing, with the root cause stated. Prepare add.js 与 add.test.js in an isolated directory before verification; never use real credentials or production data.
8. Report exact files and events. Removal may delete only .codeartsdoer/skills/diagnosing-bugs; preserve the parent, other Skills, and all user files.
```

### User prompt

```text
Install and verify user-scoped Matt Pocock diagnosing-bugs at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76 for the current Windows user.
...
3. Run exactly: $target = Join-Path $userRoot "skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("diagnosing-bugs"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\engineering\diagnosing-bugs") -Destination (Join-Path $target "diagnosing-bugs") -Recurse. This copies skills/engineering/diagnosing-bugs -> $userRoot/skills/diagnosing-bugs.
...
6. Run exactly: codearts run --auto -m <external-model-id> --format json "Use the diagnosing-bugs skill. Run node --test to reproduce the bug, diagnose the root cause, apply the smallest fix, and rerun the same command. Do not add dependencies."
7. Pass only with completed Skill "diagnosing-bugs" and the same command failing then passing, with the root cause stated. Prepare add.js 与 add.test.js in an isolated directory before verification; never use real credentials or production data.
8. Remove only $userRoot/skills/diagnosing-bugs; preserve the parent, package.json, codearts_cli.json, and unrelated Skills.
```
...
$names = @("diagnosing-bugs")
if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }
Copy-Item -LiteralPath (Join-Path $source "skills\engineering\diagnosing-bugs") -Destination (Join-Path $target "diagnosing-bugs") -Recurse
```

The pinned source is content-only: no dependencies or lifecycle scripts. Prepare add.js 与 add.test.js in an isolated directory before verification; never use real credentials or production data.
...
codearts run --auto -m "mimo/mimo-v2.5" --format json "Use the diagnosing-bugs skill. Run node --test to reproduce the bug, diagnose the root cause, apply the smallest fix, and rerun the same command. Do not add dependencies."
...
Require the expected locations, completed Skill "diagnosing-bugs", and the same command failing then passing, with the root cause stated. A plausible answer without completed `skill` events is not a pass.

## Usage

```text
Use the diagnosing-bugs skill. Run node --test to reproduce the bug, diagnose the root cause, apply the smallest fix, and rerun the same command. Do not add dependencies.
```
...
Delete only the `diagnosing-bugs` directories in the chosen scope. Preserve the parent, unrelated Skills, `package.json`, and `codearts_cli.json`. Resolve exact absolute paths first, then rerun discovery.
...
- [Pinned upstream source](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/engineering/diagnosing-bugs)
