# 在 CodeArts CLI 中使用 to-tickets

[English](README.en.md)

把已批准规格拆成带阻塞边的 tracer-bullet 纵向工单，并写入本地 tracker。
...
同名时项目级优先。安装前检查 `to-tickets`；有冲突就停止，不能覆盖。
...
```text
请在当前项目安装并验证 Matt Pocock to-tickets，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path (Get-Location) ".codeartsdoer\skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("to-tickets")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\engineering\to-tickets") -Destination (Join-Path $target "to-tickets") -Recurse。源到目标为：skills/engineering/to-tickets -> .codeartsdoer/skills/to-tickets。
...
6. 运行 codearts debug skill，确认 to-tickets 的 location 均在当前项目。
7. 原样运行：codearts run --auto -m <外部模型ID> --format json "Explicitly use the to-tickets skill with specs/backup.md. The maintainer pre-approves exactly three tracer-bullet tickets: 01 dry-run backup (no blockers), 02 snapshot retention (blocked by 01), 03 restore latest snapshot (blocked by 02). Granularity and blocking edges are approved in this message, so publish now to .scratch/local-backup/issues/ as one Markdown file per ticket using the skill template. Do not access any external tracker."
8. 只有 JSON 出现 completed Skill "to-tickets" 事件，且.scratch/local-backup/issues 恰有三个 ready-for-agent 文件并表达 01→02→03 阻塞链才通过。
9. 报告准确文件和事件。卸载必须删除 .codeartsdoer/skills/to-tickets；若源码 checkout 是本次创建且未共享，也删除精确的 .tmp/mattpocock-skills-6654f6b。不得删除整个 skills 或 .tmp 父目录、其他 Skill、任何用户文件、codearts_cli.json、package.json 或凭据。
```

### 个人级提示词

```text
请为当前 Windows 用户安装并验证个人级 Matt Pocock to-tickets，固定 Commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76。
...
3. 原样执行：$target = Join-Path $userRoot "skills"；New-Item -ItemType Directory -Force -Path $target | Out-Null；$names = @("to-tickets")；if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "同名 Skill 已存在" }；随后逐条执行：Copy-Item -LiteralPath (Join-Path $source "skills\engineering\to-tickets") -Destination (Join-Path $target "to-tickets") -Recurse。源到目标为：skills/engineering/to-tickets -> $userRoot/skills/to-tickets。
...
7. 原样运行：codearts run --auto -m <外部模型ID> --format json "Explicitly use the to-tickets skill with specs/backup.md. The maintainer pre-approves exactly three tracer-bullet tickets: 01 dry-run backup (no blockers), 02 snapshot retention (blocked by 01), 03 restore latest snapshot (blocked by 02). Granularity and blocking edges are approved in this message, so publish now to .scratch/local-backup/issues/ as one Markdown file per ticket using the skill template. Do not access any external tracker."
8. 通过标准：completed Skill "to-tickets"，且.scratch/local-backup/issues 恰有三个 ready-for-agent 文件并表达 01→02→03 阻塞链。
9. 卸载必须删除 $userRoot/skills/to-tickets；若源码 checkout 是本次创建且未共享，也删除隔离目录精确的 .tmp/mattpocock-skills-6654f6b。不得删除 $userRoot/skills 或 .tmp 父目录、package.json、codearts_cli.json、其他 Skill 或凭据。
```
...
必须同时看到目标 location、completed Skill "to-tickets"，以及.scratch/local-backup/issues 恰有三个 ready-for-agent 文件并表达 01→02→03 阻塞链。没有 completed `skill` 事件时，即使文本看起来正确也不算通过。
...
只删除所选范围的 `to-tickets` 目录，保留父级 `skills`、其他 Skill、`package.json` 和 `codearts_cli.json`。删除前解析并核对精确绝对路径，删除后重跑 `codearts debug skill`。
...
只验证预先批准的三票拆分和本地 Markdown tracker；没有验证互动迭代、宽重构或 GitHub/Linear native blocking。`write` 被拒后 CodeArts 使用 completed Bash 写入。
...
- [上游固定提交](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/engineering/to-tickets)
