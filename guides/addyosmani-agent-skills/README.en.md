# Addy Osmani Code Simplification on CodeArts CLI

[简体中文](README.md)

Install the verified `code-simplification` skill to reduce code complexity while preserving behavior.
...
```text
Install and verify the code-simplification skill from addyosmani/agent-skills for CodeArts CLI in the current project, pinned to commit df1edb2e05487d0aa6d93c747141e0aed1187f25.
...
3. Check .codeartsdoer/vendor/addyosmani-agent-skills and .codeartsdoer/skills/code-simplification. Stop and report if either exists; never overwrite.
...
5. Run codearts debug skill. Confirm code-simplification resolves under this project's .codeartsdoer/skills/code-simplification/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --sandbox --model "<selected model>" "Call the skill tool exactly once with name code-simplification. Do not use any other tool. Simplify this JavaScript without changing behavior and output only the final function: function isValid(input) { if (input.length > 0 && input.length < 100) { return true; } return false; }"
7. Passing criteria: JSON must contain exactly one completed `code-simplification` skill call. The final function must directly return the length boolean expression and contain no `if`.
...
```text
Install and verify the user-scoped code-simplification skill from addyosmani/agent-skills for the current Windows user, pinned to commit df1edb2e05487d0aa6d93c747141e0aed1187f25.

Follow exactly:
1. In PowerShell run $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only vendor/addyosmani-agent-skills and skills/code-simplification below it. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. Ask me to select a provider/model ID if ambiguous.
3. Check $userRoot/vendor/addyosmani-agent-skills and $userRoot/skills/code-simplification. Stop and report if either exists; never overwrite.
4. In the same PowerShell session run exactly:
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "df1edb2e05487d0aa6d93c747141e0aed1187f25") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "skills\code-simplification") -Destination $target -Recurse
5. From a directory with no same-named project skill, run codearts debug skill. Confirm code-simplification resolves under the current user's .codeartsdoer/skills/code-simplification/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from the same directory:
   codearts run --format json --sandbox --model "<selected model>" "Call the skill tool exactly once with name code-simplification. Do not use any other tool. Simplify this JavaScript without changing behavior and output only the final function: function isValid(input) { if (input.length > 0 && input.length < 100) { return true; } return false; }"
7. Passing criteria: JSON must contain exactly one completed `code-simplification` skill call. The final function must directly return the length boolean expression and contain no `if`.
8. Removal may include only $userRoot/skills/code-simplification and $userRoot/vendor/addyosmani-agent-skills. Report the tool event and result; do not claim success after any failure.
```
...
JSON must contain exactly one completed `code-simplification` skill call. The final function must directly return the length boolean expression and contain no `if`. A plausible final answer without a completed `skill` event is not a pass.
...
Review a new commit before updating. Remove the old skill and dedicated `vendor/addyosmani-agent-skills` in the selected scope, reinstall the new pinned commit, and repeat verification. Do not treat floating `main` as a verified version.
...
| Pinned source | [df1edb2](https://github.com/addyosmani/agent-skills/commit/df1edb2e05487d0aa6d93c747141e0aed1187f25) |
| Verified skill | `code-simplification` |
| License | MIT |
...
The installation was reproduced in two fresh projects and then verified at user scope from a directory without project configuration. Real CodeArts sessions called `code-simplification` and completed the representative task above. All three scopes were rolled back, after which the skill disappeared from discovery.

Only `code-simplification` was verified. Large refactors, real test-suite execution, and other repository skills, hooks, and commands were not tested.
...
- [Pinned skill](https://github.com/addyosmani/agent-skills/blob/df1edb2e05487d0aa6d93c747141e0aed1187f25/skills/code-simplification/SKILL.md)
