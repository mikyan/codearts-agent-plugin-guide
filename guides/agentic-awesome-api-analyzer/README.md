# 在 CodeArts CLI 中使用 api-analyzer

[English](README.en.md)

安装 `api-analyzer` Skill，用一两行检查 API 请求的方法、URL、请求头、请求体、查询参数和认证是否自洽。

## 选择安装范围

项目级 `<项目>/.codeartsdoer` 适合固定一个仓库；个人级 `~/.codeartsdoer` 适合跨项目复用。二选一；项目级同名 Skill 优先。目标 Skill 或 vendor 路径已存在时必须停止，不能覆盖。
...
```text
在当前 Windows 项目中安装并验证项目级 api-analyzer。固定来源 https://github.com/sickn33/agentic-awesome-skills.git，release v17.3.0，Commit 69906dde999aaa0f3d173f0e3d5bcdb84c87a294，源目录 skills/api-analyzer。
先在项目根运行 codearts --version、git --version、codearts models，让我选择真实可用的 <model>；不得读取或打印凭据。令 $root=Join-Path (Get-Location) '.codeartsdoer'，$source=Join-Path $root 'vendor\agentic-awesome-api-analyzer'，$target=Join-Path $root 'skills\api-analyzer'。检查 $source、$target 和 ~/.codeartsdoer/skills/api-analyzer；任一冲突就停止。只创建 $source 和 $target，不修改 package.json、codearts_cli.json、凭据、插件或其他 Skill。
...
运行 codearts debug skill；唯一生效的 api-analyzer location 必须是 $target/SKILL.md。把 <model> 换成已选 ID，原样执行：codearts run --format json --model "<model>" 'Call the skill tool exactly once with name api-analyzer. Use no other tool, do not access the network, and do not read or write files. Validate only this synthetic request: GET https://example.invalid/search with JSON body {"q":"desk"}; the endpoint is public and expects q as a query parameter. Give the concise verdict and fix required by the skill. Do not claim a live request ran.'
只有退出码 0、恰好一个来自 $target 的 name=api-analyzer/status=completed 事件、没有其他工具事件，且最终结果指出 GET 不应带 body 并把 q 移到 /search?q=desk 时通过。上游可能附加 TestMu AI HyperExecute 与文档生成询问；要原样报告，不能当成额外工具调用。报告 Commit、绝对路径和事件。卸载只能删除精确 $target 与 $source，再用 codearts debug skill 确认旧路径消失；禁止删除整个 .codeartsdoer 或任何根配置。
```

### 个人级提示词

```text
为当前 Windows 用户安装并验证个人级 api-analyzer。固定来源 https://github.com/sickn33/agentic-awesome-skills.git，release v17.3.0，Commit 69906dde999aaa0f3d173f0e3d5bcdb84c87a294，源目录 skills/api-analyzer。
运行 codearts --version、git --version、codearts models，让我选择 <model>；不得读取或打印凭据。令 $root=Join-Path ([Environment]::GetFolderPath('UserProfile')) '.codeartsdoer'，$source=Join-Path $root 'vendor\agentic-awesome-api-analyzer'，$target=Join-Path $root 'skills\api-analyzer'。确认 $source、$target 及全新 consumer 的 .codeartsdoer/skills/api-analyzer 都不存在；冲突时停止。不得修改用户 package.json、codearts_cli.json、凭据、插件或项目配置。
...
进入没有同名项目 Skill 的全新 consumer。codearts debug skill 的唯一 api-analyzer location 必须是 $target/SKILL.md。把 <model> 换成已选 ID，原样执行：codearts run --format json --model "<model>" 'Call the skill tool exactly once with name api-analyzer. Use no other tool, do not access the network, and do not read or write files. Validate only this synthetic request: GET https://example.invalid/search with JSON body {"q":"desk"}; the endpoint is public and expects q as a query parameter. Give the concise verdict and fix required by the skill. Do not claim a live request ran.' 只有退出码 0、恰好一个来自 $target 的 name=api-analyzer/status=completed 事件、没有其他工具事件，且最终结果指出 GET 不应带 body 并把 q 移到 /search?q=desk 时通过。报告 Commit、绝对路径和事件。卸载只删除精确 $target、$source 和核对后的 consumer，再用 codearts debug skill 确认旧路径消失；不得删除用户根、根配置、凭据或其他 Skill。
```
...
先用 `codearts debug skill` 核对唯一来源，再运行 Agent 提示词中的完整 `codearts run`。成功判据是退出码 0、唯一 completed `api-analyzer` 事件来自目标绝对路径、没有其他工具事件，并准确把 GET body 改成查询参数。

## 使用

```text
Use api-analyzer. 检查这条 API 请求是否正确；如果信息不足，只问一个可能改变结论的问题，并给一行修复建议。
```

## 更新

用 `git -C $source rev-parse HEAD` 核对固定版本。更新 Commit 前重新审查许可证和 `skills/api-analyzer` 全部内容，并重做两个项目、个人范围和回滚测试。

## 卸载

解析绝对路径后，只删除 `$root/skills/api-analyzer` 与 `$root/vendor/agentic-awesome-api-analyzer`，再用 `codearts debug skill` 确认旧路径消失。不得删除整个 `.codeartsdoer`、根配置或其他 Skill。
...
| 内容许可证 | Skill frontmatter：MIT；AAS 其余内容：CC BY 4.0 |
| 环境 | CodeArts CLI 26.8.1；Windows 11 Build 26200；`mimo/mimo-v2.5` |
| 范围 | 两个全新项目 + 个人级；2026-09-16 |

## 已知限制

只验证了纯文本、合成请求的只读分析，没有请求真实 API。上游正文要求宣传 TestMu AI HyperExecute 并询问是否生成 API 文档；三次实测中只有项目 B 附加了这段文字，核心校验结果不受影响。

## 安全

固定目录只有一个 4,404 字节的 `SKILL.md`，无依赖、锁文件、生命周期脚本、二进制、下载器或遥测。三次实测均只有目标 Skill 事件，没有文件、Shell 或网络访问。

## 证据与来源

- [中文研究记录](../../research/2026-09-16.md) · [English](../../research/2026-09-16.en.md)
- [固定 Skill 目录](https://github.com/sickn33/agentic-awesome-skills/tree/69906dde999aaa0f3d173f0e3d5bcdb84c87a294/skills/api-analyzer)
