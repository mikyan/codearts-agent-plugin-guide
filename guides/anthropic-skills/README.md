# 在 CodeArts CLI 中使用 Anthropic Frontend Design

[English](README.en.md)

安装经过验证的 `frontend-design` Skill，为界面设计提供主题化的色彩、字体、布局和视觉标志指导。
...
```text
请在当前项目中为 CodeArts CLI 安装并验证 anthropics/skills 的 frontend-design Skill，源码固定到 Commit 0a64e398ec6bb34a494f0c347e8ccae53a862f8e。
...
3. 检查 .codeartsdoer/vendor/anthropic-skills 和 .codeartsdoer/skills/frontend-design。任一存在就停止并报告，不得覆盖。
...
5. 运行 codearts debug skill，确认 frontend-design 的 location 位于当前项目 .codeartsdoer/skills/frontend-design/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步确认的模型，然后原样执行：
   codearts run --format json --sandbox --model "<选择的模型>" "Call the skill tool exactly once with name frontend-design. Do not use any other tool. Apply the skill to a landing page for a tea-brewing timer aimed at careful home brewers. Output exactly four labeled lines: Color, Type, Layout, Signature. The Color line must contain at least four six-digit hex colors."
7. 通过标准：JSON 中必须只有一次成功的 `frontend-design` Skill 调用；最终结果包含 `Color`、`Type`、`Layout`、`Signature` 四行，且至少包含 4 个六位十六进制颜色。
...
```text
请为当前 Windows 用户安装并验证个人级 anthropics/skills frontend-design Skill，源码固定到 Commit 0a64e398ec6bb34a494f0c347e8ccae53a862f8e。

严格执行：
1. 在 PowerShell 执行 $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer"。只修改该目录下 vendor/anthropic-skills 和 skills/frontend-design；不要修改 $userRoot/package.json、codearts_cli.json、凭据、项目配置或全局软件。
2. 运行 codearts --version、git --version 和 codearts models；模型不明确时先询问我选择 provider/model ID。
3. 检查 $userRoot/vendor/anthropic-skills 和 $userRoot/skills/frontend-design。任一存在就停止并报告，不得覆盖。
4. 在同一个 PowerShell 会话原样执行：
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "0a64e398ec6bb34a494f0c347e8ccae53a862f8e") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "skills\frontend-design") -Destination $target -Recurse
5. 在没有项目级同名 Skill 的目录运行 codearts debug skill，确认 frontend-design 的 location 位于当前用户 .codeartsdoer/skills/frontend-design/SKILL.md。
6. 把 <选择的模型> 替换为第 2 步确认的模型，在同一目录原样执行：
   codearts run --format json --sandbox --model "<选择的模型>" "Call the skill tool exactly once with name frontend-design. Do not use any other tool. Apply the skill to a landing page for a tea-brewing timer aimed at careful home brewers. Output exactly four labeled lines: Color, Type, Layout, Signature. The Color line must contain at least four six-digit hex colors."
7. 通过标准：JSON 中必须只有一次成功的 `frontend-design` Skill 调用；最终结果包含 `Color`、`Type`、`Layout`、`Signature` 四行，且至少包含 4 个六位十六进制颜色。
8. 卸载只能移除 $userRoot/skills/frontend-design 和 $userRoot/vendor/anthropic-skills。报告工具事件与结果；任一步失败不得宣称成功。
```
...
JSON 中必须只有一次成功的 `frontend-design` Skill 调用；最终结果包含 `Color`、`Type`、`Layout`、`Signature` 四行，且至少包含 4 个六位十六进制颜色。 只有最终文本但没有成功的 `skill` 工具事件，不能视为通过。
...
更新时先审查新 Commit，再移除所选范围内的旧 Skill 和专用 `vendor/anthropic-skills`，按相同步骤重新安装并复测。不要复用浮动的 `main` 作为已验证版本。
...
| --- | --- |
| 兼容状态 | **Works** |
| 上游项目 | [anthropics/skills](https://github.com/anthropics/skills) |
| 固定源码 | [0a64e39](https://github.com/anthropics/skills/commit/0a64e398ec6bb34a494f0c347e8ccae53a862f8e) |
| 已验证 Skill | `frontend-design` |
| 许可证 | Apache-2.0 |
...
该安装在两个全新项目中复现，随后又从没有项目配置的目录验证个人级加载。真实 CodeArts 会话成功调用 `frontend-design` 并完成上述代表任务；三个范围均完成回滚，回滚后 Skill 不再被发现。

只验证了 `frontend-design`；Anthropic 仓库中的其他 Skills、实际页面文件生成和浏览器视觉验收不在本次范围内。
...
- [固定版本 Skill](https://github.com/anthropics/skills/blob/0a64e398ec6bb34a494f0c347e8ccae53a862f8e/skills/frontend-design/SKILL.md)
