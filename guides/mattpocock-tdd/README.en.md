# Use tdd with CodeArts CLI

[中文](README.md)

Enforces an observed failing test before the minimum implementation and refactor.
...
Project Skills override same-named user Skills. Stop on any collision in `tdd`.
...
```text
Install and verify project-scoped Matt Pocock tdd at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76.
...
3. Run exactly: $target = Join-Path (Get-Location) ".codeartsdoer\skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("tdd"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\engineering\tdd") -Destination (Join-Path $target "tdd") -Recurse. This copies skills/engineering/tdd -> .codeartsdoer/skills/tdd.
...
6. Run exactly: codearts run --auto -m <external-model-id> --format json "Use the tdd skill. Implement clamp in clamp.js with strict red-green-refactor using node --test. Show the failing test first, then make it pass. Do not add dependencies."
7. Pass only with completed Skill "tdd" events and the same test failing first and then passing. Prepare clamp.js 与 clamp.test.js in an isolated directory before verification; never use real credentials or production data.
8. Report exact files and events. Removal may delete only .codeartsdoer/skills/tdd; preserve the parent, other Skills, and all user files.
```

### User prompt

```text
Install and verify user-scoped Matt Pocock tdd at commit 6654f6b60cd9d5be8b54c6fafe44346dabeb3b76 for the current Windows user.
...
3. Run exactly: $target = Join-Path $userRoot "skills"; New-Item -ItemType Directory -Force -Path $target | Out-Null; $names = @("tdd"); if ($names | Where-Object { Test-Path -LiteralPath (Join-Path $target $_) }) { throw "Same-name Skill exists" }; then run: Copy-Item -LiteralPath (Join-Path $source "skills\engineering\tdd") -Destination (Join-Path $target "tdd") -Recurse. This copies skills/engineering/tdd -> $userRoot/skills/tdd.
...
6. Run exactly: codearts run --auto -m <external-model-id> --format json "Use the tdd skill. Implement clamp in clamp.js with strict red-green-refactor using node --test. Show the failing test first, then make it pass. Do not add dependencies."
7. Pass only with completed Skill "tdd" and the same test failing first and then passing. Prepare clamp.js 与 clamp.test.js in an isolated directory before verification; never use real credentials or production data.
8. Remove only $userRoot/skills/tdd; preserve the parent, package.json, codearts_cli.json, and unrelated Skills.
```
...
The pinned source is content-only: no dependencies or lifecycle scripts. Prepare clamp.js 与 clamp.test.js in an isolated directory before verification; never use real credentials or production data.
...
Require the expected locations, completed Skill "tdd", and the same test failing first and then passing. A plausible answer without completed `skill` events is not a pass.
...
Delete only the `tdd` directories in the chosen scope. Preserve the parent, unrelated Skills, `package.json`, and `codearts_cli.json`. Resolve exact absolute paths first, then rerun discovery.
...
- [Pinned upstream source](https://github.com/mattpocock/skills/tree/6654f6b60cd9d5be8b54c6fafe44346dabeb3b76/skills/engineering/tdd)
