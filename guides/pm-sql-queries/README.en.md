# PM SQL Query Generation on CodeArts CLI

[简体中文](README.md)

Install the verified `sql-queries` skill from PM Skills to turn a natural-language analytics request and schema into SQL with an explanation, index advice, and validation steps.
...
```text
Install and verify the sql-queries skill from phuryn/pm-skills for CodeArts CLI in the current project, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.
...
3. Check .codeartsdoer/vendor/pm-skills-sql-queries and .codeartsdoer/skills/sql-queries. Stop and report if either exists; never overwrite.
...
5. Run codearts debug skill and confirm sql-queries resolves under this project's .codeartsdoer/skills/sql-queries/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name sql-queries and use no other tool. Do not access a database, run SQL, access the network, or write files. For PostgreSQL, tables are users(id bigint primary key, created_at timestamptz) and sessions(id bigint primary key, user_id bigint references users(id), started_at timestamptz, duration_seconds int). Generate a production query returning users created in the last 30 days with at least 5 sessions after signup, their session count, and average duration; order by count descending. Explain it, propose one index, and give a read-only validation query. Use exact markers DIALECT, SQL-QUERY, EXPLANATION, INDEX, and VALIDATION-QUERY."
7. Passing criteria: There must be one completed `sql-queries` skill event resolving to the selected native directory. The result must identify PostgreSQL, join users to post-signup sessions, filter the last 30 days and at least five sessions, calculate count and average duration, and include one index plus a read-only validation query.
8. Report the commit, source, target, completed event, resolved path, and result. Removal may delete only .codeartsdoer/skills/sql-queries and .codeartsdoer/vendor/pm-skills-sql-queries. Never delete .codeartsdoer itself, package.json, codearts_cli.json, or another skill. Stop after any failure and do not claim success.
```

### User installation prompt

```text
Install and verify the user-scoped sql-queries skill from phuryn/pm-skills for the current Windows user, pinned to commit 18468a95b427e70e258b51389796367c6f684e7d.

Follow exactly:
1. In PowerShell set $userRoot = Join-Path ([Environment]::GetFolderPath("UserProfile")) ".codeartsdoer". Modify only $userRoot/vendor/pm-skills-sql-queries and $userRoot/skills/sql-queries. Do not modify $userRoot/package.json, codearts_cli.json, credentials, project configuration, or global software.
2. Run codearts --version, git --version, and codearts models. If several models are available, ask me to select a provider/model ID.
...
4. In the same PowerShell session run exactly:
   ...
   if ((git -C $source rev-parse HEAD).Trim() -ne "18468a95b427e70e258b51389796367c6f684e7d") { throw "Unexpected commit." }
   Copy-Item -LiteralPath (Join-Path $source "pm-data-analytics\skills\sql-queries") -Destination $target -Recurse
5. Change to a clean directory with no same-named project skill. Run codearts debug skill and confirm the location is ~/.codeartsdoer/skills/sql-queries/SKILL.md.
6. Replace <selected model> with the model from step 2 and run exactly from that clean directory:
   codearts run --format json --model "<selected model>" "Call the skill tool exactly once with name sql-queries and use no other tool. Do not access a database, run SQL, access the network, or write files. For PostgreSQL, tables are users(id bigint primary key, created_at timestamptz) and sessions(id bigint primary key, user_id bigint references users(id), started_at timestamptz, duration_seconds int). Generate a production query returning users created in the last 30 days with at least 5 sessions after signup, their session count, and average duration; order by count descending. Explain it, propose one index, and give a read-only validation query. Use exact markers DIALECT, SQL-QUERY, EXPLANATION, INDEX, and VALIDATION-QUERY."
7. Passing criteria: There must be one completed `sql-queries` skill event resolving to the selected native directory. The result must identify PostgreSQL, join users to post-signup sessions, filter the last 30 days and at least five sessions, calculate count and average duration, and include one index plus a read-only validation query.
8. Report the commit, completed event, resolved path, and result. Removal may delete only $userRoot/skills/sql-queries and $userRoot/vendor/pm-skills-sql-queries. Never delete the user-root package.json, codearts_cli.json, credentials, or another skill. Stop after any failure and do not claim success.
```
...
There must be one completed `sql-queries` skill event resolving to the selected native directory. The result must identify PostgreSQL, join users to post-signup sessions, filter the last 30 days and at least five sessions, calculate count and average duration, and include one index plus a read-only validation query. A plausible final answer without the completed target `skill` event is not a pass.

## Use

```text
Call the skill tool with name sql-queries. Generate a query from this PostgreSQL schema, explain its logic, and provide a read-only validation statement.
```
...
No real database was connected and no query plan was executed; review index advice against data volume, existing indexes, and write costs.
...
- Pin the commit and copy only `pm-data-analytics/skills/sql-queries`; do not run upstream `validate_plugins.py`, tests, or other plugin content.
...
- [Pinned skill](https://github.com/phuryn/pm-skills/blob/18468a95b427e70e258b51389796367c6f684e7d/pm-data-analytics/skills/sql-queries/SKILL.md)
